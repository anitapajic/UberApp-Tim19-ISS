...
public interface PassengerRepository extends JpaRepository<Passenger, Integer> {
    ...
    // @Query(value = "select p.id, p.email, r.*, l1.*, l2.* from USER p, passenger_ride pr, ride r, ride_paths rp, path path, location l1, location l2 where P.ID = ?1 and p.id = pr.passenger_id and r.id = pr.ride_id and rp.ride_id= r.id and rp.paths_id=path.id and path.departure_id = l1.id and path.destination_id = l2.id", nativeQuery = true)
